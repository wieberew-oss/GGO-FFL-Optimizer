...
The enrichment pipeline:
  1. Match each DK player to nflverse recent stats by name + team (using dynamic aliases)
...
# ---------------------------------------------------------------------------
# Name matching helpers & Dynamic Aliases
# ---------------------------------------------------------------------------
DEFAULT_ALIASES = {
    "james cook ii": "james cook",
...
def _get_active_aliases() -> dict[str, str]:
    """Retrieves session state aliases, initializing defaults if needed."""
    if "dk_name_aliases" not in st.session_state:
        st.session_state.dk_name_aliases = DEFAULT_ALIASES.copy()
    return st.session_state.dk_name_aliases


def _normalize_name(name: str) -> str:
    """Lowercase, strip punctuation, and check active session aliases."""
    import re
    cleaned = re.sub(r"[^a-z ]", "", name.lower()).strip()
    aliases = _get_active_aliases()
...
def _find_player_row(
    ...
    """Match a DK player name to a nflverse row using dynamic aliases."""
    if stats_df.empty:
        return None
    ...
    stats_df["_norm"] = stats_df[name_col].apply(lambda x: _normalize_name(str(x)))

    match_team = stats_df[(stats_df["_norm"] == norm) & (stats_df[team_col] == team_nfl)]
    if not match_team.empty:
        return match_team.iloc[0]

    match_any_team = stats_df[stats_df["_norm"] == norm]
    if not match_any_team.empty:
        return match_any_team.iloc[0]
...
def _find_snap_row(
    ...
    """Match a player in the snap counts DataFrame using dynamic aliases."""
    if snaps_df.empty:
        return None
    ...
    snaps_df["_norm"] = snaps_df["player"].apply(lambda x: _normalize_name(str(x)))

    match_team = snaps_df[(snaps_df["_norm"] == norm) & (snaps_df["team"] == team_nfl)]
    if not match_team.empty:
        return match_team.iloc[0]

    match_any = snaps_df[snaps_df["_norm"] == norm]
    if not match_any.empty:
        return match_any.iloc[0]
